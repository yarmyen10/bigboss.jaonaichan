...
import { patchProfile } from "../../services/jaonaichan";
...
interface Props {
  profile: UserProfile;
  onProfileChange: Dispatch<SetStateAction<UserProfile | null>>;
}

export default function UserInfoCard({ profile, onProfileChange }: Props) {
  ...
  const registeredDate = profile.registeredAt
    ? new Date(profile.registeredAt).toLocaleDateString("th-TH", { year: "numeric", month: "long", day: "numeric" })
    : "";
      ...
      <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
        <div className="w-full">
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-2 lg:gap-7 2xl:gap-x-32">
            ...
            <div>
              <p className="mb-2 text-xs leading-normal text-gray-500 dark:text-gray-400">Display Name</p>
              <p className="text-sm font-medium text-gray-800 dark:text-white/90">{profile.displayName}</p>
            </div>
            ...
            <div>
              ...
              <p className="text-sm font-medium text-gray-800 dark:text-white/90">{profile.nickname || "—"}</p>
            </div>

            <div>
              <p className="mb-2 text-xs leading-normal text-gray-500 dark:text-gray-400">Registered</p>
              <p className="text-sm font-medium text-gray-800 dark:text-white/90">{registeredDate}</p>
            </div>

            {profile.description && (
              <div className="col-span-2">
                <p className="mb-2 text-xs leading-normal text-gray-500 dark:text-gray-400">Bio</p>
                <p className="text-sm font-medium text-gray-800 dark:text-white/90">{profile.description}</p>
              </div>
            )}
          </div>
        </div>
